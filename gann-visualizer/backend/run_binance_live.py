...
def run_live(symbol, interval, qty, client, momentum_filter=False, strategy_mode='target_progression'):
    ...
    for b in client.get_balance():
        if float(b.get("available_balance", 0)) > 0:
            print(f"  {b['asset']}: available={b['available_balance']}")
    print()

    interval_ms = INTERVAL_MS.get(interval.lower(), 3_600_000)
    warmup_bars = 300

    print(f"Fetching {warmup_bars} {interval} candles for initialization...")
    raw_candles = client.fetch_klines(symbol, interval, limit=warmup_bars)
    if not raw_candles:
    ...
    warmup_end = max(study.config["left_bars"] + study.config["right_bars"] + 1, len(candles) // 4)
    ...
    last_processed_time = candles[-1]["time"] if candles else 0
    ...
    print(f"Entering live loop. Interval: {interval} ({interval_ms // 1000}s). Ctrl+C to stop.")
    print(f"Watching for new bars...")
    print()

    try:
        while True:
            now_ms = int(time.time() * 1000)
            lookback_ms = now_ms - (interval_ms * 2)
            new_raw = client.fetch_klines(symbol, interval, start_time_ms=lookback_ms, limit=2)

            for c in new_raw:
                candle_ts = c["time"] // 1000
                if candle_ts <= last_processed_time:
                    continue
                candle = _candle_to_dict(c)
                candle["time"] = candle_ts
                candles.append(candle)

                bar_index = len(candles) - 1
                ts_str = datetime.fromtimestamp(candle_ts).strftime("%Y-%m-%d %H:%M:%S")
                print(f"[{ts_str}] Bar {bar_index} O:{candle['open']:.2f} H:{candle['high']:.2f} L:{candle['low']:.2f} C:{candle['close']:.2f}")

                result = study.process_bar(candles, bar_index, state=None)
                ...
                for mode in entry_modes:
                    _process_bar_events(bar_events, candles, bar_index, mode_breached[mode], mode_active[mode], mode_trades[mode],
                                        live_mode=(mode == 'retest_baseline'), client=client if mode == 'retest_baseline' else None,
                                        symbol=symbol, qty=qty,
                                        momentum_filter=False, unfiltered_trades=None,
                                        unfiltered_active_trades=None,
                                        study=study,
                                        entry_mode=mode, tracker_results=tracker_results)

                _process_bar_events_model_a(bar_events, candles, bar_index, model_a_breached, model_a_active, model_a_trades,
                ...
                last_processed_time = candle_ts

            sleep_sec = max(5, interval_ms / 2000)
    ...
    for p in open_positions:
        ...
        if amt != 0:
            try:
                ...
                qty_close = abs(amt)
                client.place_market_order(symbol, side_close, qty_close, reduce_only=True)
                print(f"Closed open position: {qty_close} {symbol} {side_close}")
            except Exception as e:
